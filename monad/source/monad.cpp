...
void importFiles(const string & preservedFolder, const string & theSourceFolder,
                 const string & destFolder, const vector<string> & files, bool exit_if_error = true);
...
int main(int argc, const char * const * argv)
{
    ...
    const vector<string> & postprocessing_commands = config["Post-Make Commands"];
...
void monad::copyRequiredFiles()
{
    ...
    // they do not exist
    importFiles("",         sourceFolder,      gradeFolder, config("Required Files", ""), false); 
...
void monad::importFiles(const string & preservedFolder, const string & theSourceFolder,
                 const string & destFolder, const vector<string> & files, bool exit_if_err /* = true */)
{
	...
	for (size_t i = 0; i < files.size(); i++)
	{
		...
		else if (theSourceFolder != "")
		{
            ...
            if (exit_if_err) 
            {
                ...
                exec("cp", "-RL", sourceFile, destFile);
            }
		}
		else continue;
